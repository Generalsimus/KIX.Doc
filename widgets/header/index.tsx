...
export function Header() {
    ...
    const onRouteChange = () => {
        state.burgerMenuStatus = false;
    }

    return <div class="header flex content-between items-center padding-container" mark={(node: HTMLDivElement) => {
        window.addEventListener('scroll', function (event) {
            if (this.scrollY > node.offsetHeight) {
                node.setAttribute("style", `
                position: fixed;
                width: 100%;
                z-index: 888888888;
                transform: translateY(-${Math.max(100 + (((node.offsetHeight - this.scrollY) / node.offsetHeight) * 100), 0)}%);
                `);
            } else {
                node.setAttribute("style", ``)
            }
        ...
        <div class="menu flex h100">
            <div class="burger-menu flex items-center h100" onClick={() => {
                state.burgerMenuStatus = !state.burgerMenuStatus
            }}>
            ...
            <div class={`menu-items flex items-center h100 ${state.burgerMenuStatus ? "active" : ""}`}>
                <switch href="/docs/" onClick={onRouteChange}>Docs</switch>
                <switch href="/examples/" onClick={onRouteChange}>Examples</switch>
                <switch href="/support/" onClick={onRouteChange}>Support KIX</switch>
                <switch href="/about/" onClick={onRouteChange}>About</switch>
            </div>
        </div>
    </div>
